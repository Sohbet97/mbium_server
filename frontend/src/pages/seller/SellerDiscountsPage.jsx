...
import {
  Plus, Pencil, Trash2, Eye, EyeOff, X, Loader2, RefreshCw,
  Tag, ShoppingCart, Package, Gift, Truck, ChevronRight, ArrowLeft,
} from 'lucide-react'
          ...
          {TYPES.map(({ value, titleKey, descKey }) => {
            ...
            return (
              ...
              >
                ...
                <ChevronRight className="h-4 w-4 text-slate-300 dark:text-white/20 shrink-0 group-hover:text-slate-400 dark:group-hover:text-white/40 transition-colors" />
              </button>
            )
          })}
        </div>
...
function SectionLabel({ children }) {
  return (
    <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-2">{children}</p>
  )
}

// ── Applies-to section (PRODUCT discounts only) ───────────────────────────────
...
function DiscountFormDrawer({ discount, category, onClose, onSaved }) {
  const { t } = useTranslation()
  ...
  async function handleSave() {
    ...
    try {
      const payload = {
        ...
        min_order_amount: hasMinOrder && form.min_order_amount ? parseFloat(form.min_order_amount) : null,
  ...
  const catTitle = t(CAT_TITLE_KEYS[cat] ?? 'seller.catOrder')
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-lg bg-white dark:bg-[#111114] shadow-2xl flex flex-col">
        ...
        <div className="shrink-0 flex items-center gap-3 px-5 py-4 border-b dark:border-white/[0.06]">
          ...
          <h2 className="font-semibold dark:text-white flex-1 truncate">
            {discount ? t('seller.editDiscountTitle') : catTitle}
          </h2>
          <button type="button" onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 text-slate-400 transition-colors">
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
          ...
          {/* Applies to — PRODUCT discounts only */}
          {cat === 'PRODUCT' && (
...
function CategoryBadge({ category, type }) {
  const meta = CATEGORY_META[category] ?? CATEGORY_META.ORDER
  const Icon = meta.icon
  return (
...
export default function SellerDiscountsPage() {
  ...
  const [drawer, setDrawer]       = useState(null) // { discount: null|obj, category: string }
  ...
  function fmtIdentifier(d) {
    if (d.method === 'AUTOMATIC') return d.name || '—'
    return d.code || '—'
  }
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      onClick={() => setDrawer({ discount: d, category: d.category })}
