...
function WarehouseFormModal({ open, onClose, onSaved, initial }) {
  ...
  const [form, setForm] = useState({ name: '', address: '', city: '', contact_phone: '', is_active: true, is_default: false })
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    if (initial) {
      setForm({ name: initial.name ?? '', address: initial.address ?? '', city: initial.city ?? '', contact_phone: initial.contact_phone ?? '', is_active: initial.is_active ?? true, is_default: initial.is_default ?? false })
    } else {
      setForm({ name: '', address: '', city: '', contact_phone: '', is_active: true, is_default: false })
    }
  }, [initial, open])
  ...
  const submit = async () => {
    ...
    try {
      if (initial) {
        await SellerApi.warehouses.update(initial.id, form)
      } else {
        await SellerApi.warehouses.create(form)
      }
      onSaved()
      onClose()
    } catch (e) {
...
function AdjustStockModal({ open, onClose, onSaved, warehouseId, levelRow }) {
  ...
  useEffect(() => {
    if (levelRow) {
      ...
      setForm({ product_id: '', variant_id: null, quantity: 1, type: 'INBOUND', note: '' })
    }
  }, [levelRow, open])
  ...
  const submit = async () => {
    setSaving(true)
    try {
      await SellerApi.warehouses.adjustInventory(warehouseId, form)
      onSaved()
      onClose()
    } catch (e) {
      toast.error(e?.response?.data?.message ?? 'Error')
...
function WarehouseDetail({ warehouse, onClose }) {
  const { t } = useTranslation()
  return (
    <Card className="mt-4">
...
export default function SellerWarehousesPage() {
  ...
  const handleDelete = async (w) => {
    ...
    try {
      await SellerApi.warehouses.delete(w.id)
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold dark:text-white flex items-center gap-2">
          <Building2 className="h-5 w-5" />{t('warehouses.myWarehouses')}
        </h1>
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {warehouses.map(w => (
            <Card
              key={w.id}
              className={`cursor-pointer transition-colors ${selected?.id === w.id ? 'ring-2 ring-primary' : ''}`}
            ...
            >
              <CardContent className="py-3 px-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm dark:text-white flex items-center gap-2">
                      {w.name}
                      {w.is_default && <Star className="h-3.5 w-3.5 text-yellow-500" />}
                      <span className={`text-xs px-1.5 py-0.5 rounded-full ${w.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                        {w.is_active ? t('common.active') : t('common.inactive')}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 mt-0.5">
                      {[w.city, w.address, w.contact_phone].filter(Boolean).join(' · ') || '—'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setFormModal({ open: true, item: w })}>
      ...
      {selected && (
        <WarehouseDetail warehouse={selected} onClose={() => setSelected(null)} />
      )}
