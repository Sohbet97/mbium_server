...
const BuyerRequestOfferService = require('./services/buyer-request-offers')
...
router.get('/buyer-requests', async (req, res, next) => {
    try {
        ...
        if (req.query.shop_id !== undefined) filter.shop_id = req.query.shop_id
...
router.get('/buyer-requests/:id', async (req, res, next) => {
    try {
        ...
        const offers = await BuyerRequestOfferService.getByRequest(req.params.id)
        return res.json({ model, offers })
    } catch (e) { next(e) }
})
